        ...
        for (int i = 0; i < floor.GetLength(0); i++)
        {
            for (int j = 0; j < floor.GetLength(1); j++)
            {
                if (floor[i, j] == 1)
                {
                    ...
                    //if (i == 6 && j == 6)
                    //{
                    //    minimapRoom = visitedRoom;
                    //}
                    //else
                    //{
                    //    minimapRoom = toVisitRoom;
                    //}

                    ////Gizmos.DrawCube(new Vector3(i + cam.transform.position.x + 15, j + cam.transform.position.y + 4), new Vector3(0.9f, 0.9f));

                    //minimap.SetTile(new Vector3Int(i + (int)cam.transform.position.x, j + (int)cam.transform.position.y, 0), minimapRoom);
                }
            }
        }
    }

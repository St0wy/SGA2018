...
public class RoomGenerator : MonoBehaviour
{
    ...
    private GameObject[] enemies = new GameObject[11];

    private int enemyNbr = 0;
    ...
    private void Update()
    {
        ...
        {
            if (enemies[i].GetComponent<Health>().health_point <= 0 && !enemies[i].GetComponent<Health>().dead)
            {
                deadMonster++;
                enemies[i].GetComponent<Health>().dead = true;
                enemies[i].GetComponent<Health>().Die();
            }
        }
    }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (highlightTile == ground)
                    {
                        ...
                        if (x == width / 2 && y == height / 2 && !startRoom)
                        {
                            GameObject temp = Instantiate(enemy, new Vector3Int(-width / 2 + x + roomOffsetX, -(-height / 2 + y + roomOffsetY) - 15, -3), Quaternion.identity);
                            enemies[enemyNbr] = temp;
                            enemyNbr++;
                        }
                    }
                    else
